...
браузеру. Активация Pages/runtime config и реальная браузерная проверка ещё
в процессе — полная UI-готовность не заявляется.
...
## Оставшийся gate публичного интерфейса
...
Подтвердить фактическую публикацию frontend runtime config и пройти реальный
браузерный Pages→API сценарий: открыть профиль, отправить одно сообщение,
получить `source: llm` и увидеть ответ без маркировки локальной демореплики.
Серверная проверка Origin не заменяет браузерный CORS. Последовательный repeat
...
До завершения Pages-проверки не объявлять публичный интерфейс готовым.
