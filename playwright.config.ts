...
export default defineConfig({
  ...
  use: {
    ...
    trace: 'off',
    video: process.env.OMNITWIN_DEMO_QA_VIDEO==='1'?{mode:'on',size:{width:1280,height:720}}:'off',
    screenshot: 'only-on-failure',
  },
  projects: [
    { name: 'desktop-1920x1080', testMatch: /(?:journey|map-interactions|visual-readiness|cohort-state|observed-reference|city-repair|city-v2|tiled-game)\.spec\.ts/,
      use: { viewport: { width: 1920, height: 1080 }, contextOptions: { screen: { width: 1920, height: 1080 }, reducedMotion: 'no-preference' } } },
    { name: 'portrait-390x844', testMatch: /(?:responsive|observed-reference|city-repair|city-v2-mobile)\.spec\.ts/,
