...
export class GameLandUseGround{
 ...
 applyPrepared(result:ReturnType<typeof prepareGameLandUseGround>,options:GameLandUseGroundOptions){return this.update([],options,result);}
 ...
 update(features:readonly GameVegetationFeature[],options:GameLandUseGroundOptions,preparedResult?:ReturnType<typeof prepareGameLandUseGround>){if(this.disposed)return this.telemetry;if(options.loading){this.telemetry.state='loading_retained';this.retain(options.origin);return this.telemetry;}
  try{const r=preparedResult??prepareGameLandUseGround(features,options,this.telemetry.retainedBytes),old=this.mesh?.geometry;if(!old||!equal(old.getAttribute('position').array,r.positions)){const geometry=new BufferGeometry();geometry.setAttribute('position',new BufferAttribute(r.positions,3));geometry.setAttribute('normal',new BufferAttribute(r.normals,3));if(r.positions.length){geometry.computeBoundingBox();geometry.computeBoundingSphere();}
