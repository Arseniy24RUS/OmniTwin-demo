...
import { createActorFootprintClearance, buildActorRoadHints, type ActorFootprintClearance } from './actorFootprintClearance';
import {GamePresentationClock} from './gameClock';
import {motionSamplingPolicy} from './motionSamplingPolicy';
...
export default function TiledGameScene(props:WorldSceneProps & {populationDatasetId?:string}) {
  ...
  useEffect(()=>{
    ...
    const start=async()=>{
      ...
      map=new maplibregl.Map({container:container.current,style,center:[c.longitude,c.latitude],zoom:c.zoom,pitch:c.pitch,bearing:c.bearing,
        minZoom:12,maxZoom:20,maxPitch:60,renderWorldCopies:false,fadeDuration:150,pixelRatio:tier==='low'?0.75:Math.min(1,devicePixelRatio),
      ...
      const laneTraffic=new LaneTraffic();let trafficContext=latest.current.trafficContextKey,trafficSeek=latest.current.presentationClock?.seekRevision??0;
      ...
      let frames=0,lastSample=-Infinity,lastSampleWall=-Infinity,lastTelemetry=0,bridge:ActorColumnsBridge|null=null;
      const motionCost={bridgeBuilds:0,bridgeMaxMs:0,guardBuilds:0,guardMaxMs:0,samples:0,sampleMaxMs:0};
      let actorCounts={people:0,vehicles:0};
      let bridgeEntities:WorldSceneProps['entities'],bridgeMotion:WorldSceneProps['mobilityPresentationMovement'];
      ...
      let clearance:ActorFootprintClearance|null=null;
      ...
      const layer=new TiledGameLayer({tilesetUrl:visualPack?.tilesetUrl??new URL('city-visual-v1/tileset.json',location.href).href,
        ...
        onReady:()=>syncCoverage(),onFrontier:()=>syncCoverage(),onDiagnostics:publish,onPick:pick});
      ...
      if(import.meta.env.DEV&&root.current)Object.defineProperty(root.current,'readGameTrafficProbe',{configurable:true,value:(ids?:readonly string[])=>laneTraffic.readProbe(ids)});
      if(import.meta.env.DEV&&root.current)Object.defineProperty(root.current,'readGameSignalProbe',{configurable:true,value:()=>laneTraffic.readSignals()});
      ...
      const syncCoverage=()=>{
        ...
        void banks.stage(snapshot,meshIds,commit=>{
          ...
          if(!committed&&banks?.diagnostics.lastError&&!disposed)setError(`Сохранена предыдущая геометрия: ${banks.diagnostics.lastError}`);
        });
      };
      ...
      const syncWater=()=>{
        ...
        try{
          ...
          const ready=layer.waterDiagnostics.polygons>0;
          for(const l of layers)if(l.type==='fill'&&l.paint?.['fill-pattern']){
      ...
      const syncActors=(force=false)=>{
        ...
        if(seek!==trafficSeek||trafficContext!==p.trafficContextKey){laneTraffic.reset();trafficSeek=seek;trafficContext=p.trafficContextKey;lastSample=-Infinity;lastSampleWall=-Infinity;}
        if(bridgeEntities!==p.entities||bridgeMotion!==p.mobilityPresentationMovement||bridgeBuildings!==p.verifiedCityBuildings||bridgeBounds!==p.verifiedCityBuildingBounds||bridgeRoads!==p.gameSourceRoads||bridgeCorridors!==p.gameSourceCorridors){
          if(bridgeBuildings!==p.verifiedCityBuildings||bridgeBounds!==p.verifiedCityBuildingBounds){
            const guardStarted=performance.now();
            clearance=p.verifiedCityBuildingBounds?createActorFootprintClearance({snapshot:p.verifiedCityBuildings,origin:visualOrigin,viewportBounds:p.verifiedCityBuildingBounds}):null;
            motionCost.guardBuilds++;motionCost.guardMaxMs=Math.max(motionCost.guardMaxMs,performance.now()-guardStarted);
          }
          const bridgeStarted=performance.now();
          bridge=new ActorColumnsBridge(p.entities??[],p.presentationMovement,p.mobilityPresentationMovement??[],visualOrigin,EPOCH,
            {sourceMetric:'provider_equirectangular_111195',onceEndpointFadeMeters:15,laneTraffic,derivePairedSidewalks:Boolean(clearance),isPointClear:clearance?.isPointClear,roadHintsByEdgeId:buildActorRoadHints(p.presentationMovement,p.gameSourceRoads??[],p.gameSourceCorridors??[])});
          motionCost.bridgeBuilds++;motionCost.bridgeMaxMs=Math.max(motionCost.bridgeMaxMs,performance.now()-bridgeStarted);
          bridgeEntities=p.entities;bridgeMotion=p.mobilityPresentationMovement;lastSample=-Infinity;lastSampleWall=-Infinity;
          ...
          if(root.current){const {offsets,...preview}=bridge.sidewalkPreview;root.current.dataset.gameSidewalkPreview=JSON.stringify({...preview,guardReady:Boolean(clearance),guard:clearance?.diagnostics??null,verifiedBounds:p.verifiedCityBuildingBounds??null});}
          actorCounts={people:(p.entities??[]).filter(e=>e.kind!=='vehicle').length,vehicles:(p.entities??[]).filter(e=>e.kind==='vehicle').length};
          if(root.current)root.current.dataset.gameTraffic=JSON.stringify(bridge.trafficPresentation);
          if(root.current)root.current.dataset.gameVehicleLanes=JSON.stringify(bridge.vehicleLanePreview);
        }
        const wall=performance.now(),seconds=readSeconds();
        const sampling=motionSamplingPolicy(clock.current.speed);
        // Keep5Hz at1x, but never span4simulated seconds across a turn at16x.
        // Sampling runs before setTime, bounded by this one scheduler.
        if(bridge&&(force||wall-lastSampleWall>=sampling.cadenceMs||seconds<lastSample||!Number.isFinite(lastSample)||(!clock.current.playing&&seconds!==lastSample))){
          const sampleStarted=performance.now();
          layer.updateActorColumns(bridge.sample(seconds,sampling.horizonSeconds),{selectedId:p.selectedId,headingEpoch:JSON.stringify([trafficContext,trafficSeek])});lastSample=seconds;lastSampleWall=wall;
          const signals=laneTraffic.readSignals();layer.updateTrafficSignals(signals);
          const limited=Boolean(signals.diagnostics.overflow);
          if(limited!==trafficLimitedValue){trafficLimitedValue=limited;setTrafficLimited(limited);}
          if(root.current)root.current.dataset.gameSignals=JSON.stringify(layer.trafficSignalDiagnostics);
          motionCost.samples++;motionCost.sampleMaxMs=Math.max(motionCost.sampleMaxMs,performance.now()-sampleStarted);
        }
        layer.setTime({presentationSeconds:seconds,playing:clock.current.playing});
      };
      ...
      resize=new ResizeObserver(()=>{activeMap.resize();surfaceRefresh?.mark();if(activeMap.loaded())publishCamera();});resize.observe(container.current);
    };
    ...
    return ()=>{disposed=true;abort.abort();if(visibilityChanged)document.removeEventListener('visibilitychange',visibilityChanged);resize?.disconnect();surfaceRefresh?.dispose();scheduler?.dispose();banks?.dispose();runtime.current=null;map?.remove();};
  ...
  useEffect(()=>{const r=runtime.current;if(!r)return;r.syncActors(true);r.scheduler.setReasonActive('timeline',!props.presentationClock?.paused);if(props.presentationClock?.paused)r.scheduler.invalidate('data');},
    [props.entities,props.mobilityPresentationMovement,props.verifiedCityBuildings,props.verifiedCityBuildingBounds,props.gameSourceRoads,props.gameSourceCorridors,props.trafficContextKey,props.presentationClock?.seekRevision,props.presentationClock?.paused,props.presentationClock?.speedMultiplier]);
