...
export class GameCourtyardGround{
  ...
  applyPrepared(result:ReturnType<typeof prepareGameCourtyardGround>,options:GameCourtyardGroundOptions){return this.update(options,result);}
  ...
  update(options:GameCourtyardGroundOptions,preparedResult?:ReturnType<typeof prepareGameCourtyardGround>){if(this.disposed)return this.telemetry;if(options.loading){this.telemetry.state='loading_retained';this.retain(options.origin);return this.telemetry;}
    try{const result=preparedResult??prepareGameCourtyardGround(options,this.telemetry.retainedBytes),old=this.mesh?.geometry,changed=!old||!equal(old.getAttribute('position').array,result.positions)||!equal(old.getAttribute('courtyardBorder').array,result.border);
