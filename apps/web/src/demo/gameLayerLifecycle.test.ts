...
function makeGl() {
  return { FRAMEBUFFER_BINDING: 1, FRAMEBUFFER: 2, VIEWPORT: 3, DEPTH_RANGE: 4,
...
describe('game layer shared-context lifecycle (contract tests, not pixel evidence)', () => {
  beforeEach(() => { harness.renderers.length = 0; harness.tiles.length = 0; });
  it('retries released detail after a desktop/mobile viewport resize at the same camera',()=>{
    ...
    layer.onAdd(map as unknown as MapLibreMap,gl as unknown as WebGL2RenderingContext);
  ...
  it('atomically returns four detailed owners to native coverage without exceeding staging or losing the next frontier announcement',()=>{
    ...
    for(let i=0;i<4;i++){
      ...
      tiles.group.add(scene);tiles.visibleTiles.add(tile);tiles.emit('load-model',{tile,scene});
  ...
  it('draws metric source roads independently of detailed building readiness and disposes their buffers',()=>{
  ...
  it('keeps independent surface rendering available when city metadata is unavailable without fetching an unverified root',()=>{
  ...
  it('selects the nearest canonical native roof even before any GLB is ready and blocks unknown native occlusion', () => {
  ...
  it('keeps the committed parent drawable while a child bank is preparing and restores an empty raw frontier', () => {
    const map=makeMap(),gl=makeGl(),onFrontier=vi.fn();
    const layer=new TiledGameLayer({tilesetUrl:'https://example.org/tileset.json',qualityTier:'low',onFrontier});
    layer.onAdd(map as unknown as MapLibreMap,gl as unknown as WebGL2RenderingContext);
    const tiles=loadCoarse(),parent=tiles.group.children[0];parent.children[0].name='committed-parent';
    layer.render(gl as unknown as WebGL2RenderingContext,input());
    ...
    tiles.group.add(scene);tiles.emit('load-model',{tile:child,scene});
  ...
  it('renders prepared actors when detailed buildings are hidden over native coverage', () => {
  ...
  it('keeps committed child geometry drawable while an empty replacement frontier awaits its native bank',()=>{
    ...
    layer.onAdd(map as unknown as MapLibreMap,gl as unknown as WebGL2RenderingContext);
    const tiles=loadCoarse(),parent=tiles.group.children[0];layer.render(gl as unknown as WebGL2RenderingContext,input());layer.commitFrontier(layer.frontier);layer.setVisible(true);
    ...
    tiles.group.add(scene);tiles.emit('load-model',{tile:child,scene});tiles.visibleTiles.clear();tiles.visibleTiles.add(child);
    layer.render(gl as unknown as WebGL2RenderingContext,input());layer.commitFrontier(layer.frontier);
  ...
  it('loads while hidden, switches only after coarse preparation, and never clears the main target', () => {
  ...
  it('low tier renders geometry without allocating or rendering any shadow target', () => {
    const map = makeMap(), gl = makeGl();
    const layer = new TiledGameLayer({ tilesetUrl: 'https://example.org/tileset.json', qualityTier: 'low' });
    layer.onAdd(map as unknown as MapLibreMap, gl as unknown as WebGL2RenderingContext);
    loadCoarse(); layer.render(gl as unknown as WebGL2RenderingContext, input()); layer.setVisible(true);
    layer.prerender(gl as unknown as WebGL2RenderingContext, input());
    layer.render(gl as unknown as WebGL2RenderingContext, input());
  ...
  it('rebuilds the PCF cache after context restoration before another main draw', () => {
    const map = makeMap(), gl = makeGl(), ready = vi.fn();
    const layer = new TiledGameLayer({ tilesetUrl: 'https://example.org/tileset.json', qualityTier: 'medium', onReady: ready });
    layer.onAdd(map as unknown as MapLibreMap, gl as unknown as WebGL2RenderingContext);
    loadCoarse(); layer.render(gl as unknown as WebGL2RenderingContext, input());
  ...
  it('retains source-road aggregate rendering outside the quarter without drawing hidden city geometry', () => {
    const map = makeMap(), gl = makeGl();
