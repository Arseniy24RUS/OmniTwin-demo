...
/** A changed request is loading immediately; neither old results nor fabricated zeroes leak into a new slice. */
function useAsyncValue<T>(request:(signal:AbortSignal)=>Promise<T>) {
 const [result,setResult]=useState<{request:typeof request;value?:T;error?:string}|null>(null);
 useEffect(()=>{
  const abort=new AbortController();
  Promise.resolve().then(()=>request(abort.signal)).then(value=>{if(!abort.signal.aborted)setResult({request,value})}).catch(error=>{if(!abort.signal.aborted)setResult({request,error:error instanceof Error?error.message:String(error)})});
  return()=>abort.abort();
 },[request]);
 const current=result?.request===request?result:null;
 return {ready:current!==null&&'value' in current,value:current?.value,error:current?.error};
}
...
function Summary({provider,context,navigate,onContextChange}:{provider:StaticDemoProvider;context:DemoContextV1;navigate:(r:DemoRoute)=>void;onContextChange:(c:Partial<DemoContextV1>)=>void}){
 ...
 const isLegacy=provider.manifest.datasetId===LEGACY_DATASET_ID&&!filtered&&context.year===2026&&context.territoryId==='RU-CHE-SET';const previousValue=previous?.population??(isLegacy?8225:null);const delta=previousValue===null?null:snapshot.population-previousValue;
...
function Inspector({selection,provider,context,onClose,onSelect,onMap}:{selection:Selection;provider:StaticDemoProvider;context:DemoContextV1;onClose:()=>void;onSelect:(s:Selection)=>void;onMap:(p:PublicFictionalPersonV1)=>void}){
 ...
 const prepare=useCallback(async (signal:AbortSignal)=>{
  ...
  else await provider.prepareVehicle(selection.id,presenceMinute,context.scenario,context.year,signal);
  return true;
 },[provider,selection.kind,selection.id,context.scenario,context.year,presenceMinute]);
 const prepared=useAsyncValue(prepare);
...
function Agents({provider,context,onContextChange,onSelect}:{provider:StaticDemoProvider;context:DemoContextV1;onContextChange:(c:Partial<DemoContextV1>)=>void;onSelect:(s:Selection)=>void}){
 ...
 const indexedSearch=provider.manifest.datasetId==='omnitwin-fictional-city-v2';

 const request=useCallback((signal:AbortSignal)=>provider.queryPeople({scenario:context.scenario,year:context.year,territoryId:context.territoryId,ageBand:context.cohort?.ageBand,sex:context.cohort?.sex,employment:context.cohort?.employment,query,offset,limit:50},signal),[provider,context.scenario,context.year,context.territoryId,context.cohort?.ageBand,context.cohort?.sex,context.cohort?.employment,query,offset]);
 const result=useAsyncValue(request);const page=result.value;
 return <section className="agents-view" data-testid="agents-view" aria-busy={!result.ready&&!result.error}><div className="page-heading"><div><p className="caption">От агрегатов к отдельному жителю</p><h1>Население в деталях</h1>{page&&<p className="muted">{number.format(page.total)} вымышленных жителей в выбранном срезе. Каждый профиль связан с аналитикой.</p>}</div></div>
 <div className="agent-filters"><label className="search-field"><Search size={18}/><input aria-label={indexedSearch?'ID жителя':'Поиск жителя'} aria-describedby={indexedSearch?'city-id-search-help':undefined} placeholder={indexedSearch?'ID жителя':'Имя, занятие или ID'} value={query} onChange={e=>setQuery(e.target.value)}/></label><label>Пол<select aria-label="Пол жителя" value={context.cohort?.sex||''} onChange={e=>onContextChange({cohort:{...context.cohort,sex:e.target.value as 'male'|'female'||undefined}})}><option value="">Все</option><option value="male">Мужчины</option><option value="female">Женщины</option></select></label><label>Возраст<select aria-label="Возрастная группа" value={context.cohort?.ageBand||''} onChange={e=>onContextChange({cohort:{...context.cohort,ageBand:(e.target.value||undefined) as NonNullable<DemoContextV1['cohort']>['ageBand']}})}><option value="">Все группы</option>{['0-17','18-34','35-54','55-69','70+'].map(x=><option key={x}>{x}</option>)}</select></label>{context.cohort&&<button onClick={()=>onContextChange({cohort:null})}>Сбросить группу</button>}</div>
 {indexedSearch&&<p id="city-id-search-help" className="caption">Поиск по ID; имя и занятие — в карточке</p>}
 {result.error?<p role="alert">Не удалось загрузить жителей: {result.error}</p>:!page?<p role="status">Загружаем жителей выбранного среза…</p>:<><div className="table-scroll"><table><thead><tr><th>Житель</th><th>Возраст</th><th>Занятие</th><th>Территория</th><th>Домохозяйство</th><th/></tr></thead><tbody>{page.items.map(p=><tr key={p.id}><td><button className="person-link" onClick={()=>onSelect({kind:'person',id:p.id})}>{p.name}</button><small className="table-id">{p.id}</small></td><td>{p.age}</td><td>{p.occupation}</td><td>{p.territoryName}</td><td>{p.householdSize === null ? '—' : `${p.householdSize} чел.`}</td><td><button aria-label={`Открыть профиль ${p.name}`} className="icon-button" onClick={()=>onSelect({kind:'person',id:p.id})}><ChevronRight size={18}/></button></td></tr>)}</tbody></table></div>{!page.items.length&&<p className="muted">В выбранном срезе жители не найдены.</p>}<div className="pagination"><button onClick={()=>setOffset(x=>Math.max(0,x-50))} disabled={offset===0}><ChevronLeft size={16}/>Предыдущие</button><span>{page.items.length?page.offset+1:0}–{page.items.length?page.offset+page.items.length:0} из {number.format(page.total)}</span><button disabled={page.nextOffset===null} onClick={()=>onContextChange({agentOffset:page.nextOffset!})}>Следующие<ChevronRight size={16}/></button></div></>}</section>;
}
function About({provider}:{provider:StaticDemoProvider}){return <article className="about-view"><p className="caption">Исследовательская платформа</p><h1>OmniTwin: от населения страны<br/>к повседневной жизни человека</h1><p className="about-lead">Агентная модель помогает исследовать демографические изменения. Карта, аналитика и сценарии — разные способы изучать одно состояние населения.</p><div className="about-columns"><section><h2>Что работает в демонстрации</h2><ul><li>Потоковая 3D-карта Челябинска и визуальный синтез городской жизни.</li><li>Связанные профили, агрегаты, возрастно-половая структура и CSV-экспорт.</li><li>Сравнение подготовленных иллюстративных сценариев.</li><li>Разговор с вымышленным жителем: серверный ИИ при подключённом сервисе, подписанные демореплики без него.</li></ul></section><section><h2>Что ещё предстоит исследовать</h2><ul><li>Завершить национальный расчёт и независимую научную валидацию.</li><li>Оценить неопределённость и прогнозную способность на отложенных данных.</li><li>Подключить проверенные национальные результаты к этим интерфейсам.</li></ul></section></div><section className="about-disclosure"><h2>Честная граница демонстрации</h2><p>На первом экране и в режиме «Статистика города» показаны опубликованные агрегаты Росстата / Челябинскстата с датами и источниками. Это наблюдаемые оценки, не результаты нашей модели. Все интерактивные жители вымышлены; их отдельный набор из {number.format(provider.manifest.initialPopulation)} персонажей в начальном срезе не является переписью или репрезентативной выборкой города. Сценарии 2026–2036 годов относятся только к этому демонстрационному набору и не являются прогнозом Челябинска. Научная модель ещё не проходит все критерии; эта версия не предназначена для официального прогноза.</p><p>География: OpenStreetMap / OpenFreeMap. Внешний вид, размещение жителей и движение — визуальный синтез. Логотип РУДН использован для представления заявки на конкурс, не как утверждение об одобрении проекта университетом.</p></section><p>Автор: <a href="https://sitkovskiy.ru/" target="_blank" rel="noreferrer">Ситковский А.М.</a> · <a href="https://github.com/Arseniy24RUS/OmniTwin-demo" target="_blank" rel="noreferrer">Исходный код демо</a></p></article>}
