...
export function decodeLocation(hash:string):{route:DemoRoute;context:DemoContextV1;selection:string|null}{
  ...
  const scenario=p.get('scenario');const age=p.get('age');const sex=p.get('sex'); const weather=p.get('weather');const employment=p.get('employment');
  ...
  return {route:path in ROUTES?path as DemoRoute:'world',selection:p.get('selected'),context:{...DEFAULT_CONTEXT,
    ...
    ...(p.has('offset')?{agentOffset:Math.floor(bounded(p.get('offset'),0,0,10000000))}:{}),
    ...
    camera:{longitude:bounded(p.get('lon'),DEFAULT_CONTEXT.camera.longitude,61.1,61.8),latitude:bounded(p.get('lat'),DEFAULT_CONTEXT.camera.latitude,54.95,55.4),zoom:bounded(p.get('zoom'),DEFAULT_CONTEXT.camera.zoom,11,20),pitch:bounded(p.get('pitch'),DEFAULT_CONTEXT.camera.pitch,0,60),bearing:bounded(p.get('bearing'),DEFAULT_CONTEXT.camera.bearing,-180,180)}
  }};
}
